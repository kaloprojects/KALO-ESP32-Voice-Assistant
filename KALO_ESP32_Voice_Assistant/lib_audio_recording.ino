...
#define BITS_PER_SAMPLE         8     // 16 bit and 8bit supported (24 or 32 bits not supported)
                                      // 8bit: working well for fast SpeechToText calls, but Audio.h can't play (library issue?)
                                      // hint: 8bit is less critical for STT services than a low 8kHz sample rate

#define GAIN_BOOSTER_I2S        32    // original I2S streams seem VERY silent, so we added an optional GAIN booster:
                                      // multiplier, typical values: 1-64, 32 seems best for INMP441 microphone (about 15 db)
